...
class BaseProfile:
    ...
    def __init__(self, r_bin_min, r_bin_max, N_bins, halo_edge=None, epsilon=0):
        ...
        self.halo_edge = halo_edge or r_bin_max
    ...
    def _get_sigma_r_bins(self, r_bins, rho_bins, phi_bins):
        ...
        return np.sqrt((_delta_sigma_rho_bins[-1] - _delta_sigma_rho_bins) / rho_bins)
    ...
    def get_profiles(self):
        ...
        r_bins = np.logspace(
            np.log10(self.r_bin_min),
            np.log10(self.r_bin_max),
            self.N_bins,
            dtype=np.float64,
        )
        ...
        conv_mass_bins, conv_phi_bins, conv_sigma_r_bins = self._get_simple_profiles(
            r_bins, conv_rho_bins
        )
    ...
    def reconstruct_density(self, conv_phi_bins, eta_bins, f_eta_bins):
        ...
        _lin_log_eddington_interp = self._get_interp(eta_bins, np.log(f_eta_bins))
...
class BaseICs(BaseProfile):
    ...
    def _get_N_part(self, N200, r200):
        ...
        _m_halo = np.exp(self.log_log_mass_interp(np.log(self.r_sample_max))) - np.exp(
            self.log_log_mass_interp(np.log(self.r_sample_min))
        )

        _m_200 = np.exp(self.log_log_mass_interp(np.log(r200))) - np.exp(
            self.log_log_mass_interp(np.log(self.r_sample_min))
        )
    ...
    def _sample_particle_positions(self):
        ...
        part_phi = 2 * np.pi * np.random.uniform(0, 1, self.N_part).astype(np.float64)  # noqa: NPY002
        part_theta = np.arcsin(
            2 * np.random.uniform(0, 1, self.N_part).astype(np.float64) - 1  # noqa: NPY002
        )
    ...
    def _Eddington_velocity_distribution(self, v, psi, rho):
        ...
        return (
            ...
            * np.pi
            * v**2
            * np.exp(self.lin_log_eddington_interp(psi - v**2 / 2))
            / rho
        )
    ...
    def _sample_particle_velocities(self, part_r):
        ...
        """
        _interp_rho_bins = np.exp(
            (np.log(self.rho_bins[:-1]) + np.log(self.rho_bins[1:])) / 2
        )
        ...
        for i in tqdm(range(self.N_bins - 1), desc="Sampling velocities"):
            ...
            _v_bins = np.linspace(
                0, _interp_vmax_bins[i], self.N_bins, dtype=np.float64
            )
            ...
            part_v[_bin_mask] = _lin_lin_inverse_P_v_interp(
                np.random.uniform(0, 1, np.sum(_bin_mask)).astype(np.float64)  # noqa: NPY002
            )

        part_v_phi = 2 * np.pi * np.random.uniform(0, 1, self.N_part).astype(np.float64)  # noqa: NPY002
        part_v_theta = np.arcsin(
            2 * np.random.uniform(0, 1, self.N_part).astype(np.float64) - 1  # noqa: NPY002
        )

        return (
            part_v * np.cos(part_v_theta) * np.cos(part_v_phi),
    ...
    def generate_ICs(self, output=None):
        ...
        if output:
            _writter = ICsWriter(
                part_coords=np.vstack([part_x, part_y, part_z]).T,
                part_velocs=np.vstack([part_vx, part_vy, part_vz]).T,
                part_ids=np.arange(1, self.N_part + 1, dtype=np.int64),
                part_masses=np.full(self.N_part, part_mass, dtype=np.float64),
            )
            _writter.write(output)

        return part_mass, part_x, part_y, part_z, part_vx, part_vy, part_vz
